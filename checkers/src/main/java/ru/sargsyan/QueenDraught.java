...
public class QueenDraught extends Draught {
    ...
    public Position hasMove(BoardHandler bh) {
        for (int i = 2; i < Board.BOARD_SIZE; ++i) {
            ArrayList<Position> freeCells = bh.freeCellsInDistance(this.getPos(), i);
            for (Position free : freeCells) {
                ArrayList<Position> between = Board.getCellsInBetween(this.getPos(), free);
                ...
                if (enemyPos != null) {
                    if (!getKilledPos().isEmpty() && getKilledPos().contains(enemyPos)) {
                        continue;
                    }
                    return enemyPos;
                }
    ...
    public Position move(Position p, Mood m, BoardHandler bh) {
        ...
        if (m == Mood.Eat) {
            ...
            ArrayList<Position> between = Board.getCellsInBetween(this.getPos(), p);
            for (Position dest : between) {
                if (bh.getFigureColor(dest) == DraughtColor.rev(this.getColor())) {
